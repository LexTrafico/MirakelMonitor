...
void RefreshGridABit(HWND hWnd, RECT * rect)
{
	int SI;
...
int DetermineDataType(char* data)
{
	...
	strcpy_s(tbuffer, 1000, data);
	k = strlen(tbuffer);
	strupr_s(tbuffer, 1000);
	//is it boolean?
...
void DisplayColumn(HWND hWnd, int SI, int c, int offset, HFONT hfont, HFONT hcolumnheadingfont)
{
	...
	int r;
	char buffer[1000];
	int iDataType, iProtection, iChanged;
	RECT trect;
	HBRUSH holdbrush;
	HPEN holdpen;
	...
	while (r <= BGHS[SI].bottomvisiblerow)
	{
		...
		else
		{
			...
			iChanged = SendMessage(hWnd, BGM_GETCHANGED, (UINT)&BGcell, 0);
			if (BGHS[SI].DRAWHIGHLIGHT)//highlight on
			{
				...
				else
				{
					...
					{
						hbrush = CreateSolidBrush(BGHS[SI].changedcolor);
					}
					else
					{
...
void CloseEdit(HWND hWnd, int SI)
{
	...
	strcpy_s(BGHS[SI].editstring, 305, "");
	RefreshGrid(hWnd);
...
void DisplayEditString(HWND hWnd, int SI, char* tstring)
{
	...
	{
		strcat_s(BGHS[SI].editstring, 305, tstring);
		strcpy_s(BGHS[SI].editstringdisplay, 305, BGHS[SI].editstring);
	}
	else
	{
		MessageBeep(0);
...
ATOM RegisterGridClass(HINSTANCE hInstance)
{
	...
	for (j = 0; j < MAX_GRIDS; j++)
	{
		...
		BGHS[j].changedcolor = RGB(255, 200, 200);
		...
		strcpy_s(BGHS[j].editstring, 305, "");
...
int FindLongestLine(HDC hdc, char* text, SIZE* size)
{
	...
	strcpy_s(temptext, 1000, text);
	p = strtok_s(temptext, 1000, "\n");
	while (p)
	{
...
static LRESULT CALLBACK GridProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SETTEXT:
	{
		...
		{
			strcpy_s(BGHS[SelfIndex].title, 305, "Title too long (300 chars max)");
		}
		else
		{
			strcpy_s(BGHS[SelfIndex].title, 305, (char*)lParam);
		}
	...
	case BGM_PROTECTCELL:
		...
		wsprintf_s(cbuffer, 1000, "%05d-%03d", LPBGcell->row, LPBGcell->col);
		//see if that cell is already loaded
		FindResult = BinarySearchListBox(BGHS[SelfIndex].hlist1, cbuffer);
		if (FindResult != LB_ERR)
		{
			//it was found, get the text, modify text delete it from list, add modified to list
			SendMessage(BGHS[SelfIndex].hlist1, LB_GETTEXT, FindResult, (long)cbuffer);
			if ((BOOL)lParam)
			{
				cbuffer[10] = 'P';
			}
			...
			SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, FindResult, (long)cbuffer);
		}
		else
		{
			//protecting or unprotecting a cell that isn't in the list
			//add it as blank;
			strcat_s(cbuffer, 1000, "|");
			if ((BOOL)lParam)
			{
				strcat_s(cbuffer, 1000, "PA");
			}
			else
			{
				strcat_s(cbuffer, 1000, "UA");
			}
			strcat_s(cbuffer, 1000, "|");
			SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, FindResult, (long)cbuffer);
		}
	...
	case BGM_SETNUMBEROFCOLS:
		BGHS[SelfIndex].cols = (int)wParam;
		break;

	case BGM_SETPROTECT:
		if ((BOOL)wParam)
		{
			strcpy_s(BGHS[SelfIndex].protect, 2, "P");
		}
		else
		{
			strcpy_s(BGHS[SelfIndex].protect, 2, "U");
		}
		break;
	...
	case BGM_SETCELLDATA:
		...
		//now add it
		strcat_s(cbuffer, 1000, "|");
		strcat_s(cbuffer, 1000, BGHS[SelfIndex].protect);
		//determine data type (text,numeric, or boolean)(1,2,3)
		iDataType = DetermineDataType((char*)lParam);
		if (iDataType == 1)
		{
			strcat_s(cbuffer, 1000, "A");
		}
		if (iDataType == 2)
		{
			strcat_s(cbuffer, 1000, "N");
		}
		if (iDataType == 3)
		{
			strcat_s(cbuffer, 1000, "T");
		}
		if (iDataType == 4)
		{
			strcat_s(cbuffer, 1000, "F");
		}
		if (iDataType == 5)
		{
			strcat_s(cbuffer, 1000, "G");
		}

		strcat_s(cbuffer, 1000, "|");
		strcat_s(cbuffer, 1000, (char*)lParam);
		FindResult = SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, 0, (long)cbuffer);
		...
		if (BGHS[SelfIndex].AUTOROW)
		{
			...
			if (j > 0)
			{
				SendMessage(BGHS[SelfIndex].hlist1, LB_GETTEXT, j - 1, (long)cbuffer);
				cbuffer[5] = 0x00;
				j = atoi(cbuffer);
		...
		if ((BGHS[SelfIndex].COLAUTOWIDTH) || (LPBGcell->row == 0))
		{
			...
			{
				count = 1;
				strcpy_s(tbuffer, 255, (char*)lParam);
				for (j = 0; j < (int)strlen(tbuffer); j++)
				{
					if (tbuffer[j] == '\n')
					{
						count++;
	...
	case BGM_GETCELLDATA:
		...
		if (FindResult != LB_ERR)
		{
			...
			cbuffer[c] = 0x00;
			strcpy_s((char*)lParam, 1000, cbuffer);
		}
		else
		{
			strcpy_s((char*)lParam, 1000, "");
		}
		break;
	...
	case BGM_DELETECELL:
		...
		wsprintf(cbuffer, "%05d-%03d", LPBGcell->row, LPBGcell->col);
		//see if that cell is already loaded
		FindResult = BinarySearchListBox(BGHS[SelfIndex].hlist1, cbuffer);
		if (FindResult != LB_ERR)
		{
	...
	case WM_GETDLGCODE:
		...
		if (wParam == VK_ESCAPE)
		{
			...
			{
				BGHS[SelfIndex].EDITING = FALSE;
				strcpy_s(BGHS[SelfIndex].editstring, 305, "");
				HideCaret(hWnd);
				RefreshGrid(hWnd);
	...
	case WM_KEYDOWN:
		...
		if (wParam == VK_UP)
		{
			...
			SetHomeRow(hWnd, SelfIndex, BGHS[SelfIndex].cursorrow, BGHS[SelfIndex].cursorcol);
			SetHomeCol(hWnd, SelfIndex, BGHS[SelfIndex].cursorrow, BGHS[SelfIndex].cursorcol);
			rect.top = BGHS[SelfIndex].cursorrow - BGHS[SelfIndex].rowheight;
			rect.bottom = BGHS[SelfIndex].cursorrow + BGHS[SelfIndex].rowheight;
			rect.left = 0;
			rect.right = 300;
			RefreshGridABit(hWnd, &rect);
			break;
		}
	...
	case WM_CREATE:
		...
		if ((BG_GridIndex >= 0) && (BG_GridIndex < MAX_GRIDS))//if you aren't over the MAX_GRIDS limit, add a grid
		{
			...
			strcpy_s(BGHS[BG_GridIndex].title, 305, lpcs->lpszName);
